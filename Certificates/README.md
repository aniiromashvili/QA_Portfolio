# Professional Certifications

This repository directory documents my formal education, professional training, and industry certifications within the Software Quality Assurance and Agile methodologies domain.

---

## Credentials List

| Certification / Course | Issuing Organization | Status / Year |
| :--- | :--- | :--- |
| Manual QA Pro | GenofIT Academy | Completed (2026) |
| Theoretical Manual Testing Course | GenofIT Academy | Completed (2025) |
---

## Professional Value

Continuous learning is core to my engineering philosophy. These certifications validate my foundational knowledge in:
* Modern software testing methodologies (Functional, Regression, and UI/UX testing).
* Bug life cycle tracking and professional documentation.
* Agile/Scrum framework integration within cross-functional tech teams.
